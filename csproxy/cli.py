...
from .utils import CSProxyError, Config, setup_logger
...
def main_proxy(argv=None):
    ...
    try:
        ...
        if crashed:
            logger.warning(
                f"Detected {len(crashed)} crashed tunnel(s): "
                + ", ".join(
                    str(t.get("port", t.get("local_port", t.get("id", "?")))) for t in crashed
                )
            )
...
def main_serve(argv=None):
    ...
    sp_redir.add_argument(
        "code", nargs="?", type=int, default=302, help="HTTP redirect code (default: 302)"
    )
