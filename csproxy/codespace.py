...
class CodespaceSelector:
    ...
    def _get_machine_types(self, repo: str) -> list:
        ...
        result = self.gh.run_gh_command(
            ["api", f"/repos/{repo}/codespaces/machines", "--jq", '[.machines[].name] | join(" ")'],
            check=False,
        )
    ...
    def _create_and_wait(self, repo: str, location: str = "") -> str:
        ...
        if not name:
            ...
            codespaces = self.gh.list_codespaces()
            if codespaces:
                codespaces_sorted = sorted(
                    codespaces, key=lambda x: x.get("createdAt", ""), reverse=True
                )
    ...
    def _wait_until_available(self, name: str) -> None:
        ...
        while time.time() - start < self.READY_TIMEOUT_SECS:
            ...
            if state == "Available":
                ...
                self.gh.run_gh_command(
                    ["api", f"/user/codespaces/{name}/start", "-X", "POST"], check=False
                )
            else:
        ...
        raise TimeoutError(
            f"Codespace '{name}' did not become available within {self.READY_TIMEOUT_SECS}s"
        )
