...
from .wg_setup import (  # noqa: F401
    ...
    build_remote_setup_script as _build_remote_setup_script,
...
from .wg_constants import (  # noqa: F401
    WG_INTERFACE,
    WG_PORT,
    WG_LOCAL_IP,
    WG_REMOTE_IP,
...
def start_tunnel(config: Config, gh: GitHubManager) -> None:
    ...
    result = _run_gh(
        ["codespace", "ssh", "-c", cs_name, "--", "echo SSH OK"],
        sudo_user=sudo_user,
        capture_output=True,
        text=True,
    )
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Failed to copy setup script to codespace: " f"{str(result.stderr).strip()}"
            )
        ...
        if copy_attempted:
            ...
            wipe_cmd = [
                ...
                cs_name,
                "--",
                f"shred -u {shlex.quote(setup_path)} 2>/dev/null || "
                f"rm -f {shlex.quote(setup_path)}",
            ]
            if sudo_user and os.geteuid() == 0:
    ...
    verify_cmd = [
        ...
        cs_name,
        "--",
        f"pgrep -f 'socat.*TCP-LISTEN.*{shlex.quote(str(TCP_RELAY_PORT))}' "
        "> /dev/null && echo 'relay running'",
    ]
    if sudo_user and os.geteuid() == 0:
    ...
    socat_proc = subprocess.Popen(
        ["socat", f"UDP-LISTEN:{WG_PORT},reuseaddr,fork", f"TCP:127.0.0.1:{TCP_RELAY_PORT}"]
    )
    ...
    result = runner.run(
        ["ping", "-c", "1", "-W", "3", remote_host], capture_output=True, timeout=10
    )
    if result.returncode == 0:
...
def stop_tunnel(config: Config, gh: GitHubManager) -> None:
    ...
    if f"0.0.0.0/1.*{WG_INTERFACE}" in result.stdout or re.search(
        rf"0\.0\.0\.0/1.*{re.escape(WG_INTERFACE)}", result.stdout
    ):
        logger.info("Restoring routing...")
        runner.run(["ip", "route", "del", "0.0.0.0/1", "dev", WG_INTERFACE], capture_output=True)
        runner.run(["ip", "route", "del", "128.0.0.0/1", "dev", WG_INTERFACE], capture_output=True)
    ...
    if result.returncode == 0:
        if os.geteuid() != 0:
            logger.warning("Need root to fully clean up. Run: sudo cs-wg down")
        else:
            runner.run(["ip", "link", "del", WG_INTERFACE], capture_output=True)
    ...
    runner.run(
        ["pkill", "-f", f"ssh.*{shlex.quote(str(TCP_RELAY_PORT))}:127.0.0.1"], capture_output=True
    )
...
def show_status(config: Config, gh: GitHubManager) -> None:
    ...
    if result.returncode == 0:
        ...
        print()
        result = runner.run(
            ["ping", "-c", "1", "-W", "2", remote_host], capture_output=True, timeout=10
        )
        if result.returncode == 0:
